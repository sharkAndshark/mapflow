...
| UI-017 | 公开嵌入页 | /tiles/:slug/embed **无需认证**，提供真正可嵌入第三方页面的 iframe viewer。页面自身消费 `/tiles/:slug/meta` 与公开瓦片端点；标准 CRS、custom CRS、PNG/MVT 使用与文档页一致的预览逻辑；PMTiles 使用 PMTiles-aware client；404 时显示明确错误而不是空白 | iframe 页面可加载并发起公开瓦片/PMTiles 请求；错误态可见 | `frontend/tests/publish.spec.js frontend/tests/custom-crs.spec.js` | E2E | P1 |
