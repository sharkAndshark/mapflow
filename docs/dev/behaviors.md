...
| UI-007 | 发布按钮 | 文件列表每行显示"发布/复制/取消发布"操作按钮（仅 ready 状态），已发布文件显示"复制"和"取消发布" | 按钮状态正确 | `npm run test:e2e` | E2E | P0 |
| UI-008 | 发布弹窗 | 点击"发布"打开模态框，显示文件名、slug 输入框（默认文件 ID）、公开地址预览，提交后更新列表 | 弹窗交互正确 | `npm run test:e2e` | E2E | P0 |
