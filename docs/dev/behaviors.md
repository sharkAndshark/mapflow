...
| API-019 | PostGIS 源注册 | POST /api/postgis/sources/register 需要认证。请求体包含 `connectionName/schema/object/geometryColumn/fidColumn` 等，校验 relation 与列合法后创建 `files` 记录（`type=postgis`,`tileSource=postgis`,`status=ready`）并写入 `postgis_connections/postgis_sources/dataset_columns` | 201 + `{fileId,status}` / 400（对象或列校验失败） / 401 / 500（如 APP_SECRET 缺失） | `bash scripts/test/postgis-integration.sh` | Integration | P0 |
