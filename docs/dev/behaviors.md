...
| API-010 | 公开瓦片 | GET /tiles/:slug/:z/:x/:y **无需认证**，验证 `public_slug` 存在且 `is_public=TRUE`。动态生成返回 MVT（属性键根据 useAliases 设置决定使用别名或原始名称）；MBTiles 返回 MVT 或 PNG。空瓦片返回 204 No Content | 200 + MVT/PNG / 204（空瓦片） / 400 / 404 | `cargo test test_public_tiles_*` + `cargo test test_public_tile_use_aliases_*` | Integration | P0 |
