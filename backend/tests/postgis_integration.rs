...
static APP_SECRET_INIT: Once = Once::new();
...
async fn setup_app() -> (
    ...
    APP_SECRET_INIT.call_once(|| {
        APP_SECRET.get_or_init(|| "postgis-test-secret".to_string());
    });
    let secret = APP_SECRET.get().expect("APP_SECRET");
    conn.execute(
